document.addEventListener('DOMContentLoaded', function () {
    ...
    const populateBulkAddModal = () => {
        ...
        state.financialData.forEach(yearData => {
            ...
            yearData.months.forEach(monthData => {
                ...
                // monthData.monthが数字のみの場合は「月」を追加、既に「月」が含まれている場合はそのまま
                const monthText = String(monthData.month);
                label.textContent = monthText.includes('月') ? monthText : `${monthText}月`;
    ...
    async function loadAndRenderData(enteredPassword = null) {
        ...
        if (storedName) {
            ...
            if (userNameMobile) {
                userNameMobile.textContent = state.userName;
            }
        }
    ...
    // データ分析機能
    // ========================================
    const dataAnalysisButton = document.getElementById('data-analysis-button');
    const dataAnalysisModal = document.getElementById('data-analysis-modal');
    const dataAnalysisCloseBtn = document.getElementById('data-analysis-close-btn');
    ...
    // データ分析モーダルを閉じる
    if (dataAnalysisCloseBtn) {
        dataAnalysisCloseBtn.addEventListener('click', () => {
            hideModal(dataAnalysisModal);
            destroyAllCharts();
        });
    }
    ...
    function destroyAllCharts() {
        ...
        if (incomeBreakdownChart) { incomeBreakdownChart.destroy(); incomeBreakdownChart = null; }
        if (expenditureBreakdownChart) { expenditureBreakdownChart.destroy(); expenditureBreakdownChart = null; }
    }
        ...
        state.financialData.forEach(year => {
            year.months.forEach(month => {
                months.push({
                    ...
                    expenditure: month.totalExpenditure || 0,
                    balance: month.finalBalance || 0,
                    incomeItems: month.income || [],
                    expenditureItems: month.expenditure || []
                });
            });
        });
        return months;
    ...
    function renderAnalysisCharts() {
        ...
        // 収入内訳（円グラフ）
        const allIncomeItems = monthlyData.flatMap(m => m.incomeItems);
        const incomeCategories = aggregateByCategory(allIncomeItems);
        const incomeBreakdownCtx = document.getElementById('income-breakdown-chart').getContext('2d');
        ...
        // 支出内訳（円グラフ）
        const allExpenditureItems = monthlyData.flatMap(m => m.expenditureItems);
        const expenditureCategories = aggregateByCategory(allExpenditureItems);
        const expenditureBreakdownCtx = document.getElementById('expenditure-breakdown-chart').getContext('2d');
    ...
    function calculateAndDisplayStats() {
        ...
        if (monthlyData.length === 0) {
            document.getElementById('stat-avg-income').textContent = '¥0';
            document.getElementById('stat-avg-expenditure').textContent = '¥0';
            document.getElementById('stat-savings-rate').textContent = '0%';
            document.getElementById('stat-max-expenditure').textContent = '-';
            return;
        }
        ...
        const avgExpenditure = totalExpenditure / monthlyData.length;
        const savingsRate = totalIncome > 0 ? ((totalIncome - totalExpenditure) / totalIncome * 100) : 0;

        let maxExpenditureMonth = monthlyData[0];
        monthlyData.forEach(m => {
            if (m.expenditure > maxExpenditureMonth.expenditure) {
                maxExpenditureMonth = m;
            }
        });

        document.getElementById('stat-avg-income').textContent = formatter.format(avgIncome);
        document.getElementById('stat-avg-expenditure').textContent = formatter.format(avgExpenditure);
        document.getElementById('stat-savings-rate').textContent = savingsRate.toFixed(1) + '%';
        document.getElementById('stat-max-expenditure').textContent = maxExpenditureMonth.label;
    }
});
